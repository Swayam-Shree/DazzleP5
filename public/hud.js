...
function makeHud() {
	let s = function (sketch) {
		sketch.setup = function () {
			...
			youtube_player.p.style("z-index", "2");
			sketch.ui_buttons = [];
			...
			sketch.pages[2].on = !window.localStorage.getItem("isFirstLogin");;
			window.localStorage.setItem("isFirstLogin", "visited");
			...
			textspray_picker.style("outline", "none");
			textspray_picker.style("padding", "none");
			textspray_picker.style("color", color(150));
			textspray_picker.style("background-color", color(30));
			textspray_picker.style("font-size", "18px");
			textspray_picker.style("font-family", "Lucida Console");
			textspray_picker.position(50,120); 
			textspray_picker.hide() ; 
			textspray_picker.hidden = true ;
			sketch.ui_buttons.push(new Button(sketch, "T", 14, () => {
				textspray_picker.hidden = !textspray_picker.hidden ; 
				textspray_picker.hidden ? textspray_picker.hide() : textspray_picker.show() ;
			}, false, 20 , 120, 0));
			color_picker = new ColorPicker(sketch);
			color_picker.show_text = true;
